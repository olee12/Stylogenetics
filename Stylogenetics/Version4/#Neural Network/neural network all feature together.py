...
def getTestVector(feature_words,raw_text):
    if len(feature_words[0].split(" ")) > 1:
        return getTestVector_two_word(feature_words,raw_text);
...
def getTestVector_two_word(feature_words,raw_text):
    formated_text = formatText(raw_text);
    words = getWordList(formated_text);
    words = [word for word in words if len(word)>1];
    words = [word for word in words if isEnglish(word)==False];
...
def MakeTrainDataForSVM(feature_number,feature_words,atleast=300):
    train_data = [];
    train_label = [];
    dir = "C:\\Users\\Tahmidolee\\Documents\\Project 300\\Stylogenetics\\Stylogenetics\\Version4\\Train Data\\";
    ...
    for writer in folders:
        ...
        for file in files:
            ...
            fw.close();
            # print(file);
            vec = MakeVectorForSVM(doc,feature_number,feature_words)
...
def MakeTestDataForSVM(feature_number,feature_words,atleast = 300):
    test_data = [];
    test_label = [];
    dir = "C:\\Users\\Tahmidolee\\Documents\\Project 300\\Stylogenetics\\Stylogenetics\\Version4\\Test Data\\";
    folders = os.listdir(dir);
    for writer in folders:
        if writer.find(".") != -1:
            continue;
...
def MakeValidationDataForSVM(feature_number,feature_words,atleast = 300):
    test_data = [];
    test_label = [];
    dir = "C:\\Users\\Tahmidolee\\Documents\\Project 300\\Stylogenetics\\Stylogenetics\\Version4\\Validation Data\\";
    folders = os.listdir(dir);
    for writer in folders:
        if writer.find(".") != -1:
            continue;
